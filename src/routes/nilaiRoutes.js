...
`;
  ...
  router.get('/materi-ujian-tulis', async (req, res, next) => {
    const db = require('../../db');
    const { tahun_ajaran_id, kategori_evaluasi_id, tingkat } = req.query;

    if (!tahun_ajaran_id || !kategori_evaluasi_id || tingkat === undefined) {
      return res.status(400).json({ error: 'tahun_ajaran_id, kategori_evaluasi_id, dan tingkat wajib disertakan.' });
    }

    try {
      const key = `materi_ujian_tulis_${tahun_ajaran_id}_${kategori_evaluasi_id}_${tingkat}`;
      const result = await db.query('SELECT value FROM system_settings WHERE key = $1 LIMIT 1', [key]);
      ...
      const mapelRes = await db.query(`
        ...
        WHERE mt.tingkat = $1
          ...
          AND mp.jenis = 'Reguler'
        ORDER BY mp.nama
      `, [parseInt(tingkat, 10), tahun_ajaran_id, kategori_evaluasi_id]);
  ...
  router.post('/materi-ujian-tulis', async (req, res, next) => {
    const db = require('../../db');
    const { tahun_ajaran_id, kategori_evaluasi_id, tingkat, data } = req.body;

    if (!tahun_ajaran_id || !kategori_evaluasi_id || tingkat === undefined || !Array.isArray(data)) {
      return res.status(400).json({ error: 'tahun_ajaran_id, kategori_evaluasi_id, tingkat, dan data (array) wajib diisi.' });
    }
