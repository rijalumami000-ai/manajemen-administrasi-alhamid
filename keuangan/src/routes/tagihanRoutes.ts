...
import { pool } from '../db';
import {
  generateTagihanSPPBulanan,
  generateTagihanDaftarUlang,
  generateTagihanEvent,
...
export function registerTagihanRoutes(app: Application): void {

  // GET /api/keuangan/tagihan — Daftar tagihan dengan filter
  app.get('/api/keuangan/tagihan', authenticateToken, requireLaporanKeuangan,
    async (req: Request, res: Response) => {
      try {
        const {
          tahun_ajaran_id, santri_id, status,
          jenis_iuran_id, bulan, tahun, kategori,
          kelas_diniyah_id, kelas_sekolah_id, kode_iuran,
          page = 1, limit = 50,
        } = req.query;
        ...
        if (bulan) {
          ...
          params.push(validateBulan(bulan));
        }
        if (tahun) {
          conditions.push(`t.periode_tahun = $${idx++}`);
          params.push(validateTahun(tahun));
        }
        if (kategori) {
        ...
        if (kelas_diniyah_id) {
          ...
          params.push(validatePositiveInt(kelas_diniyah_id, 'kelas_diniyah_id'));
        }
        if (kelas_sekolah_id) {
          conditions.push(`sta.kelas_sekolah_id = $${idx++}`);
          params.push(validatePositiveInt(kelas_sekolah_id, 'kelas_sekolah_id'));
        }
        if (kode_iuran) {
  ...
  app.post('/api/keuangan/tagihan/generate-spp', authenticateToken, requireKeuangan,
    async (req: Request, res: Response) => {
      try {
        const { tahun_ajaran_id, bulan, tahun } = req.body;
        const params = {
          tahun_ajaran_id: validatePositiveInt(tahun_ajaran_id, 'tahun_ajaran_id'),
          bulan: validateBulan(bulan),
          tahun: validateTahun(tahun),
        };

        const isAdmin = req.user!.role === 'admin';
        const hasil = await generateTagihanSPPBulanan(params, req.user!.id, req.ip);
  ...
  app.post('/api/keuangan/tagihan/generate-du', authenticateToken, requireKeuangan,
    async (req: Request, res: Response) => {
      try {
        ...
        const hasil = await generateTagihanDaftarUlang(params, req.user!.id, req.ip);
  ...
  app.post('/api/keuangan/tagihan/generate-event', authenticateToken, requireKeuangan,
    async (req: Request, res: Response) => {
      try {
        const { tahun_ajaran_id, jenis_iuran_id } = req.body;
        const params = {
          tahun_ajaran_id: validatePositiveInt(tahun_ajaran_id, 'tahun_ajaran_id'),
          jenis_iuran_id: validatePositiveInt(jenis_iuran_id, 'jenis_iuran_id'),
        };

        const hasil = await generateTagihanEvent(params, req.user!.id, req.ip);
  ...
  app.post('/api/keuangan/tagihan', authenticateToken, requireKeuangan,
    async (req: Request, res: Response) => {
      try {
        const {
          santri_id, jenis_iuran_id, tahun_ajaran_id,
          nominal_tagihan, nominal_diskon = 0,
  ...
  app.put('/api/keuangan/tagihan/:id', authenticateToken, requireKeuangan,
    async (req: Request, res: Response) => {
      try {
        ...
        try {
          ...
          if (!old.rowCount) return res.status(404).json({ error: 'Tagihan tidak ditemukan.' });
          ...
          const result = await client.query(
            `UPDATE tagihan
             ...
             WHERE id=$3
             RETURNING *`,
            [nominal_diskon ?? old.rows[0].nominal_diskon, catatan ?? old.rows[0].catatan, id]
          );
          await logAudit({
  ...
  app.get('/api/keuangan/santri/:id/ringkasan', authenticateToken, requireLaporanKeuangan,
    async (req: Request, res: Response) => {
      try {
        const santriId = validatePositiveInt(req.params.id, 'id');
        const taId = req.query.tahun_ajaran_id
          ? validatePositiveInt(req.query.tahun_ajaran_id, 'tahun_ajaran_id')
          : null;
        ...
        const tagihan = await pool.query(
          `SELECT t.*,
           ...
           GROUP BY t.id, ji.id
           ORDER BY t.periode_tahun DESC, t.periode_bulan DESC, ji.urutan`,
          params
        );

        const pembayaran = await pool.query(
          `SELECT p.*, ji.nama AS nama_iuran, u.full_name AS nama_bendahara
           FROM pembayaran p
        ...
        const totalTagihan = rows.reduce((s: number, r: Record<string, unknown>) => s + Number(r.nominal_tagihan), 0);
        const totalDibayar = rows.reduce((s: number, r: Record<string, unknown>) => s + Number(r.total_dibayar), 0);
        ...
        return res.json({
          ...
          summary: {
            ...
            jumlah_tagihan_belum_lunas: rows.filter((r: Record<string, unknown>) => r.status === 'belum_lunas').length,
            jumlah_tagihan_dibebaskan: rows.filter((r: Record<string, unknown>) => r.status === 'dibebaskan').length,
          },
          tagihan: rows,
