...
import { pool } from '../db';
import { validatePositiveInt, validateBulan, validateTahun, ValidationError } from '../utils/validators';
import { NAMA_BULAN } from '../types/keuangan';

// eslint-disable-next-line @typescript-eslint/no-require-imports
const { authenticateToken } = require('../../../src/middleware/authMiddleware');
import { requireLaporanKeuangan } from '../middleware/roleGuard';

export function registerLaporanRoutes(app: Application): void {

  // ════════════════════════════════════════════════════════════
  // LAPORAN SPP PER BULAN
  // ════════════════════════════════════════════════════════════
  app.get('/api/keuangan/laporan/spp/bulanan', authenticateToken, requireLaporanKeuangan,
    async (req: Request, res: Response) => {
      try {
        const taId = validatePositiveInt(req.query.tahun_ajaran_id, 'tahun_ajaran_id');
        const bulan = validateBulan(req.query.bulan);
        const tahun = validateTahun(req.query.tahun);
        ...
        const summary = {
          ...
          target_makan: rows.reduce((s: number, r: Record<string, unknown>) => s + Number(r.nominal_tagihan_makan), 0),
          terkumpul_makan: rows.reduce((s: number, r: Record<string, unknown>) => s + Number(r.nominal_dibayar_makan), 0),
          target_madin: rows.reduce((s: number, r: Record<string, unknown>) => s + Number(r.nominal_tagihan_madin), 0),
          terkumpul_madin: rows.reduce((s: number, r: Record<string, unknown>) => s + Number(r.nominal_dibayar_madin), 0),
          santri_bebas_makan: rows.filter((r: Record<string, unknown>) => r.status_makan === 'dibebaskan').length,
          santri_bebas_madin: rows.filter((r: Record<string, unknown>) => r.status_madin === 'dibebaskan').length,
          santri_lunas_makan: rows.filter((r: Record<string, unknown>) => r.status_makan === 'lunas').length,
          santri_lunas_madin: rows.filter((r: Record<string, unknown>) => r.status_madin === 'lunas').length,
  ...
  // ════════════════════════════════════════════════════════════
  app.get('/api/keuangan/laporan/spp/semester', authenticateToken, requireLaporanKeuangan,
    async (req: Request, res: Response) => {
      try {
        const taId = validatePositiveInt(req.query.tahun_ajaran_id, 'tahun_ajaran_id');
        const semester = req.query.semester as string; // 'ganjil' | 'genap'
        ...
        const taResult = await pool.query(
          `SELECT id, kode, tahun_mulai FROM tahun_ajaran WHERE id=$1`, [taId]
        );
        if (!taResult.rows.length) return res.status(404).json({ error: 'Tahun ajaran tidak ditemukan.' });
        ...
        const result = await pool.query(
          `SELECT
           ...
           ORDER BY COALESCE(sta.nama, s.nama), t.periode_bulan`,
          [taId, bulanList, tahunKal]
        );

        // Restrukturisasi: per santri
        const santriMap: Record<number, Record<string, unknown>> = {};
        for (const row of result.rows) {
          ...
          const santri = santriMap[key] as Record<string, Record<number, unknown>>;
          if (row.kode_iuran === 'SPP_MAKAN') {
            ...
            (santriMap[key] as Record<string, number>).total_tagihan_makan += Number(row.nominal_tagihan);
            (santriMap[key] as Record<string, number>).total_dibayar_makan += Number(row.nominal_dibayar);
          } else if (row.kode_iuran === 'SPP_MADIN') {
            santri.madin_per_bulan[row.bulan] = {
              tagihan: Number(row.nominal_tagihan), dibayar: Number(row.nominal_dibayar), status: row.status
            };
            (santriMap[key] as Record<string, number>).total_tagihan_madin += Number(row.nominal_tagihan);
            (santriMap[key] as Record<string, number>).total_dibayar_madin += Number(row.nominal_dibayar);
          }
        }

        const perSantri = Object.values(santriMap).map(s => {
          const sm = s as Record<string, number>;
          return {
            ...s,
        ...
        const grandTargetMakan = perSantri.reduce((a, s) => a + (s as Record<string, number>).total_tagihan_makan, 0);
        const grandTerkumpulMakan = perSantri.reduce((a, s) => a + (s as Record<string, number>).total_dibayar_makan, 0);
        const grandTargetMadin = perSantri.reduce((a, s) => a + (s as Record<string, number>).total_tagihan_madin, 0);
        const grandTerkumpulMadin = perSantri.reduce((a, s) => a + (s as Record<string, number>).total_dibayar_madin, 0);
        ...
        return res.json({
          ...
          nama_bulan_list: bulanList.map(b => NAMA_BULAN[b]),
  ...
  // ════════════════════════════════════════════════════════════
  app.get('/api/keuangan/laporan/spp/tahunan', authenticateToken, requireLaporanKeuangan,
    async (req: Request, res: Response) => {
      try {
        const taId = validatePositiveInt(req.query.tahun_ajaran_id, 'tahun_ajaran_id');
        ...
        const perSantri = result.rows.map((r: Record<string, unknown>) => {
        ...
        const grandTargetMakan = perSantri.reduce((a: number, s: Record<string, unknown>) => a + Number(s.target_makan), 0);
        const grandDibayarMakan = perSantri.reduce((a: number, s: Record<string, unknown>) => a + Number(s.dibayar_makan), 0);
        const grandTargetMadin = perSantri.reduce((a: number, s: Record<string, unknown>) => a + Number(s.target_madin), 0);
        const grandDibayarMadin = perSantri.reduce((a: number, s: Record<string, unknown>) => a + Number(s.dibayar_madin), 0);
        const grandTarget = grandTargetMakan + grandTargetMadin;
        const grandTerkumpul = grandDibayarMakan + grandDibayarMadin;

        const taResult = await pool.query(`SELECT kode FROM tahun_ajaran WHERE id=$1`, [taId]);
        ...
        return res.json({
          ...
          summary: {
            ...
            santri_lunas_penuh: perSantri.filter((s: Record<string, unknown>) => s.status_keseluruhan === 'lunas_penuh').length,
            santri_menunggak: perSantri.filter((s: Record<string, unknown>) => s.status_keseluruhan === 'masih_menunggak').length,
  ...
  // ════════════════════════════════════════════════════════════
  app.get('/api/keuangan/laporan/daftar-ulang', authenticateToken, requireLaporanKeuangan,
    async (req: Request, res: Response) => {
      try {
        const taId = validatePositiveInt(req.query.tahun_ajaran_id, 'tahun_ajaran_id');
        const tipe = req.query.tipe as string; // 'baru' | 'lama'
        ...
        const result = await pool.query(
          `SELECT
           ...
           ORDER BY ji.urutan, COALESCE(sta.nama, s.nama)`,
          [taId, kategori]
        );
        ...
        for (const row of result.rows) {
          ...
          const item = itemMap[row.kode] as Record<string, number>;
          if (row.tagihan_id) {
            item.jumlah_santri++;
            item.target += Number(row.nominal_tagihan);
            item.terkumpul += Number(row.nominal_dibayar);
            if (row.status === 'lunas') item.jumlah_lunas++;
            else if (row.status === 'sebagian') item.jumlah_sebagian++;
            else item.jumlah_belum++;
          }
          ...
          if (row.santri_id) {
            ...
            const s = santriMap[row.santri_id] as Record<string, Record<string, unknown>>;
            s.status_per_item[row.kode] = {
              status: row.status,
              tagihan: Number(row.nominal_tagihan),
              dibayar: Number(row.nominal_dibayar),
            };
            (santriMap[row.santri_id] as Record<string, number>).total_tagihan += Number(row.nominal_tagihan);
            (santriMap[row.santri_id] as Record<string, number>).total_dibayar += Number(row.nominal_dibayar);
          }
        }

        const summaryPerItem = Object.values(itemMap)
          .sort((a, b) => Number((a as Record<string, number>).urutan) - Number((b as Record<string, number>).urutan))
          .map(item => {
        ...
        const grandTarget = summaryPerItem.reduce((a, i) => a + Number((i as Record<string, number>).target), 0);
        const grandTerkumpul = summaryPerItem.reduce((a, i) => a + Number((i as Record<string, number>).terkumpul), 0);
  ...
  // LAPORAN EVENT
  // ════════════════════════════════════════════════════════════
  app.get('/api/keuangan/laporan/event', authenticateToken, requireLaporanKeuangan,
    async (req: Request, res: Response) => {
      try {
        const taId = validatePositiveInt(req.query.tahun_ajaran_id, 'tahun_ajaran_id');
        ...
        for (const row of result.rows) {
          ...
          const ev = eventMap[row.kode] as Record<string, number | unknown[]>;
          if (row.tagihan_id) {
            (ev.jumlah_santri as unknown as number);
            (ev as Record<string, number>).jumlah_santri++;
            (ev as Record<string, number>).target += Number(row.nominal_tagihan);
            (ev as Record<string, number>).terkumpul += Number(row.nominal_dibayar);
            if (row.status === 'lunas') (ev as Record<string, number>).jumlah_lunas++;
            else if (row.status === 'sebagian') (ev as Record<string, number>).jumlah_sebagian++;
            else (ev as Record<string, number>).jumlah_belum++;
            (ev.detail_santri as unknown[]).push({
              santri_id: row.santri_id,
        ...
        const grandTarget = perEvent.reduce((a, e) => a + Number((e as Record<string, number>).target), 0);
        const grandTerkumpul = perEvent.reduce((a, e) => a + Number((e as Record<string, number>).terkumpul), 0);
  ...
  // DASHBOARD SUMMARY
  // ════════════════════════════════════════════════════════════
  app.get('/api/keuangan/dashboard', authenticateToken, requireLaporanKeuangan,
    async (req: Request, res: Response) => {
      try {
        const taId = validatePositiveInt(req.query.tahun_ajaran_id, 'tahun_ajaran_id');
        const now = new Date();
        ...
        const masuks = await pool.query(
          `SELECT
           ...
           GROUP BY ji.kategori`,
          [taId, bulanLalu, tahunLalu]
        );

        // Pengeluaran bulan ini
        const keluars = await pool.query(
          `SELECT
             jenis_kas,
        ...
        // Tunggakan SPP
        const tunggakan = await pool.query(
          `SELECT
        ...
        for (const r of masuks.rows) {
          ...
          } else if (r.kategori.startsWith('daftar_ulang')) breakdownMasuk.daftar_ulang += Number(r.bulan_ini);
          else if (r.kategori === 'event') breakdownMasuk.event += Number(r.bulan_ini);
        }
        ...
        for (const r of keluars.rows) {
          ...
          if (r.jenis_kas in breakdownKeluar) breakdownKeluar[r.jenis_kas as keyof typeof breakdownKeluar] += Number(r.bulan_ini);
        }

        const totalTunggakan = tunggakan.rows.reduce((a: number, r: Record<string, unknown>) => a + Number(r.total_tunggakan), 0);

        return res.json({
          bulan: bulanIni,
          tahun: tahunIni,
          nama_bulan: NAMA_BULAN[bulanIni],
  ...
  // ════════════════════════════════════════════════════════════
  app.get('/api/keuangan/laporan/tunggakan', authenticateToken, requireLaporanKeuangan,
    async (req: Request, res: Response) => {
      try {
        const taId = validatePositiveInt(req.query.tahun_ajaran_id, 'tahun_ajaran_id');
        ...
        const total = result.rows.reduce((a: number, r: Record<string, unknown>) => a + Number(r.total_tunggakan), 0);
